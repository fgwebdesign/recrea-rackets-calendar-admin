...
### 1. `VenueSelector.tsx` - Selector Multi-Sede
...
export function VenueSelector({ selectedVenues, onChange }: VenueSelectorProps) {
  ...
  // Fetch venues con canchas
  useEffect(() => {
    fetch('/api/venues?include_courts=true')
      .then(res => res.json())
      .then(data => {
  ...
  // Verificar si una sede está seleccionada
  const isVenueSelected = (venueId: string) => 
    selectedVenues.some(v => v.venue_id === venueId);

  // Verificar si una cancha está seleccionada
  ...
  // Obtener la sede primaria actual
  ...
  const toggleVenue = (venueId: string) => {
    if (isVenueSelected(venueId)) {
      ...
      // Si era la primaria, asignar la primera como primaria
      if (getPrimaryVenueId() === venueId && newVenues.length > 0) {
        newVenues[0].is_primary = true;
      }
      onChange(newVenues);
    } else {
      // Agregar sede (expandirla automáticamente)
      setExpandedVenues(prev => new Set([...prev, venueId]));
      onChange([...selectedVenues, {
        venue_id: venueId,
        court_ids: [],
        is_primary: selectedVenues.length === 0 // Primera sede es primaria
      }]);
    }
  };
  ...
  // Contar canchas totales seleccionadas
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">Seleccionar Sedes y Canchas</h3>
      <p className="text-sm text-gray-500">
        Selecciona las sedes donde se jugará la liga y las canchas disponibles.
      </p>
        ...
        {venues.map(venue => (
          <div 
            ...
            className={`border rounded-lg p-4 ${
              isVenueSelected(venue.id) ? 'border-green-500 bg-green-50' : 'border-gray-200'
            }`}
          >
            {/* Header de la sede */}
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      <label htmlFor={`primary-${venue.id}`} className="text-sm">
                        ⭐ Sede Primaria
                      </label>
                    </div>
                  </RadioGroup>
            ...
            {isVenueSelected(venue.id) && (
              <div className="mt-3 ml-6 space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  ...
                  de {venue.courts_count} canchas seleccionadas
                </p>
              </div>
            )}
          </div>
...
### 2. `MatchTimesEditor.tsx` - Editor de Horarios
...
export function MatchTimesEditor({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Horarios rápidos */}
      <div>
        <p className="text-sm text-gray-600 mb-2">Horarios comunes:</p>
        <div className="flex flex-wrap gap-2">
          {commonTimes
...
### 3. `LeagueTypeSelector.tsx` - Tipo de Liga

```tsx
import { RadioGroup, RadioGroupItem } from '@/components/ui/radio-group';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

type LeagueType = 'round_robin' | 'knockout' | 'groups' | 'custom';
...
export function LeagueTypeSelector({
  leagueType,
  rounds,
  frequency,
  teamSize,
  ...
  // Calcular fechas según tipo de liga
  const calculateDates = () => {
    if (leagueType === 'round_robin') {
      const gamesPerTeam = teamSize - 1;
      const totalDates = gamesPerTeam * rounds;
      return totalDates;
    }
    return '—';
  };
  ...
  const leagueTypes = [
    { 
      ...
      description: 'Cada equipo juega contra todos los demás' 
    },
    { 
      value: 'knockout' as const, 
      label: 'Eliminación directa', 
      description: 'El perdedor queda eliminado' 
    },
    { 
      value: 'groups' as const, 
      label: 'Fase de grupos + Playoffs', 
      description: 'Grupos clasificatorios y luego eliminatorias' 
    },
    { 
      value: 'custom' as const, 
      label: 'Personalizado', 
      description: 'Configuración manual de partidos' 
    }
  ];
  ...
  return (
    <div className="space-y-6">
      {/* Tipo de competencia */}
      <div className="space-y-3">
        <label className="text-sm font-medium">Tipo de Competencia</label>
        <RadioGroup 
          value={leagueType}
          onValueChange={onLeagueTypeChange}
          className="space-y-2"
        >
          {leagueTypes.map(type => (
            <div 
              ...
              className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer ${
                leagueType === type.value ? 'border-green-500 bg-green-50' : 'border-gray-200'
              }`}
            >
      ...
      {/* Vueltas (solo para round robin) */}
      {leagueType === 'round_robin' && (
        <div className="space-y-3">
          <label className="text-sm font-medium">Vueltas</label>
          <RadioGroup 
            value={rounds.toString()}
            onValueChange={(v) => onRoundsChange(parseInt(v) as 1 | 2)}
            className="flex gap-4"
          >
            <div className={`flex items-center gap-2 p-3 border rounded-lg cursor-pointer ${
              rounds === 1 ? 'border-green-500 bg-green-50' : 'border-gray-200'
            }`}>
              <RadioGroupItem value="1" id="rounds-1" />
              <label htmlFor="rounds-1" className="cursor-pointer">Solo Ida</label>
            </div>
            <div className={`flex items-center gap-2 p-3 border rounded-lg cursor-pointer ${
              rounds === 2 ? 'border-green-500 bg-green-50' : 'border-gray-200'
            }`}>
              <RadioGroupItem value="2" id="rounds-2" />
              <label htmlFor="rounds-2" className="cursor-pointer">Ida y Vuelta</label>
            </div>
          </RadioGroup>
          
          <p className="text-sm text-gray-500">
            ℹ️ Con {teamSize} equipos: 
            {rounds === 1 
              ? ` ${teamSize - 1} fechas` 
              : ` ${(teamSize - 1) * 2} fechas`}
          </p>
      ...
      {/* Frecuencia */}
      <div className="space-y-3">
        <label className="text-sm font-medium">Frecuencia de Partidos</label>
        <Select value={frequency} onValueChange={onFrequencyChange}>
          ...
          <SelectContent>
            ...
            <SelectItem value="mensual">Mensual (cada 28 días)</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
        <p className="text-sm">
          📅 <strong>Total de fechas:</strong> {calculateDates()} fechas
          <br />
          <span className="text-gray-600">
            Frecuencia: {frequency === 'semanal' ? 'cada semana' : 
                         frequency === 'quincenal' ? 'cada 2 semanas' : 'cada mes'}
          </span>
        </p>
      </div>
    </div>
...
### 4. `useVenues.ts` - Hook para Sedes
...
export function useVenues(options: UseVenuesOptions = {}) {
  ...
  const fetchVenues = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Error fetching venues');
...
### 5. Integración en `CreateLeagueForm`

```tsx
// Ejemplo de cómo integrar los nuevos componentes en el formulario existente

import { useState } from 'react';
import { VenueSelector } from '@/components/leagues/VenueSelector';
import { MatchTimesEditor } from '@/components/leagues/MatchTimesEditor';
...
export function CreateLeagueForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Paso 1 - NUEVOS
    league_type: 'round_robin' as const,
    rounds: 1 as 1 | 2,
    
    // Paso 2 - Días y frecuencia
    start_date: '',
    end_date: '',
    frequency: 'quincenal' as const,
    category_days: {} as Record<string, string>,
    
    // Paso 3 - NUEVO: Sedes
    venues: [] as VenueConfig[],
    
    // Paso 4 - NUEVO: Horarios
    match_times: ['21:30', '22:15'] as string[],
    courts_per_time_slot: 2,
  });
    ...
    switch (step) {
      ...
      case 3:
        return formData.venues.length > 0 && 
               formData.venues.every(v => v.court_ids.length > 0);
      case 4:
        return formData.match_times.length > 0;
      default:
        return true;
    }
  };
  ...
  const handleSubmit = async () => {
    const payload = {
      ...formData,
      courts_available: formData.venues.reduce(
        (sum, v) => sum + v.court_ids.length, 0
      ),
      time_slots: [[21, 24]], // Mantener compatibilidad
    };
    
    const response = await fetch('/api/leagues/createLeague', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="space-y-6">
        {step === 1 && (
          <>
            {/* Campos existentes: nombre, descripción, categorías... */}
            <LeagueTypeSelector
              leagueType={formData.league_type}
              rounds={formData.rounds}
              frequency={formData.frequency}
              teamSize={formData.team_size}
              onLeagueTypeChange={(v) => setFormData({...formData, league_type: v})}
              onRoundsChange={(v) => setFormData({...formData, rounds: v})}
        ...
        {step === 2 && (
          <>
            {/* Fechas, frecuencia, días por categoría */}
          </>
        )}
        ...
        {step === 4 && (
          <>
            <MatchTimesEditor
              matchTimes={formData.match_times}
              courtsPerSlot={formData.courts_per_time_slot}
            ...
            <div className="p-4 bg-gray-50 rounded-lg border mt-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><strong>Tipo:</strong> {formData.league_type}</div>
                <div><strong>Categorías:</strong> {formData.categories.length}</div>
                <div><strong>Sedes:</strong> {formData.venues.length}</div>
                <div><strong>Canchas:</strong> {formData.venues.reduce((s,v) => s + v.court_ids.length, 0)}</div>
                <div><strong>Horarios:</strong> {formData.match_times.join(', ')}</div>
              </div>
            </div>
          </>
        )}
