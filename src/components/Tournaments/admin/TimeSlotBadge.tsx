...
import { Clock, Sun, Moon, Sunset, Coffee } from 'lucide-react'
...
export function TimeSlotBadge({ 
  ...
  // Determinar el emoji basado en el horario
  const getTimeEmoji = () => {
    if (slotId.includes('morning') || slotId.includes('8') || slotId.includes('9') || slotId.includes('10') || slotId.includes('11')) {
      return '☀️'
    }
  ...
  const prefix = variant === 'restriction' ? '🚫' : variant === 'available' ? '✅' : getTimeEmoji()

  return (
    <Badge 
      variant="outline" 
      className={`${getVariantStyles()} ${getSizeStyles()} font-medium transition-colors`}
    >
      {getTimeIcon()}
      {prefix} {label}
    </Badge>
