...
export function FootballLeagueCard({ league }: { league: FootballLeague }) {
  ...
  return (
    <Link href={`/football/leagues/${league.id}`}>
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-2">
            <div className="p-3 rounded-lg bg-emerald-100 dark:bg-emerald-900/30">
              ...
              <p className="text-sm text-emerald-800 dark:text-emerald-200">{formatDate(league.start_date)}</p>
            </div>
            <div className="p-3 rounded-lg bg-red-100 dark:bg-red-900/30">
              <p className="text-sm font-medium text-red-900 dark:text-red-300">Fecha de fin</p>
              <p className="text-sm text-red-800 dark:text-red-200">{formatDate(league.end_date)}</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Equipos registrados</span>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-400">{registrationProgress}% completado</span>
                {league.status === 'Inscribiendo' && availableSpots > 0 ? (
                  ...
                  registeredTeams === league.team_size && (
                    <span className="text-xs font-medium text-blue-600 dark:text-blue-400">Cupos completos</span>
                  )
                )}
              </div>
            </div>
          ...
          {league.inscription_cost > 0 && (
            <div className="mt-2 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-900/20">
              <p className="text-sm font-medium text-emerald-700 dark:text-emerald-300 mb-1">Costo de inscripción</p>
              <span className="text-lg font-semibold text-emerald-600 dark:text-emerald-400">${league.inscription_cost}</span>
            </div>
          )}
        </div>
      </div>
