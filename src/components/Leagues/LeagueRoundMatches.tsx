...
export function LeagueRoundMatches({ matches, onMatchUpdate }: LeagueRoundMatchesProps) {
  ...
  const handleSaveResult = async (matchId: string, result: MatchResultFromModal) => {
    try {
      setIsLoading(true)
      ...
      const updatedMatch = {
        ...
        team1_tie1_won: result.team1_tie1_won,
        team2_tie1_won: result.team2_tie1_won,
        team1_tie2_won: result.team1_tie2_won,
        team2_tie2_won: result.team2_tie2_won,
        team1_tie3_won: result.team1_tie3_won,
        team2_tie3_won: result.team2_tie3_won
      }
  ...
  const noGroupMatches = matches.filter(m => !m.group_name);
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-[#1D283A] flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600 dark:text-gray-400">
              {match.time_slot || 'Hora no asignada'}
            </span>
          </div>
  ...
  const renderMatchesGroup = (groupMatches: LeagueMatch[], title?: string, groupColor?: string) => (
      ...
      {title && (
        <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border ${
          ...
          <Badge variant="outline" className={`ml-auto ${
            groupColor === 'blue'
              ? 'border-blue-200 dark:border-blue-500/20 bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400'
              : 'border-purple-200 dark:border-purple-500/20 bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400'
          }`}>
    ...
    <div className="space-y-6">
      ...
      {hasGroups ? (
        <div className="space-y-8">
          {groupAMatches.length > 0 && renderMatchesGroup(groupAMatches, 'A', 'blue')}
          {groupBMatches.length > 0 && renderMatchesGroup(groupBMatches, 'B', 'purple')}
        </div>
      ) : (
        <div className="grid gap-4">
          {noGroupMatches.map(renderMatchCard)}
        </div>
      )}
